...
or run: pythonw VAT_Compliance_App_V8.pyw
...
This is the entry point PyInstaller freezes for the MSI/Intune deployment
build (see the packaging notes under tools/). It's otherwise identical to
VAT_Compliance_App_V6.pyw - kept as a separate file rather than edited in
place because V6 is still the actively developed source that
tools/build_single_file_v6.py reads from to regenerate the shareable
VAT_Compliance_App_V7.pyw; the one deliberate difference is where output
files are written (see OUTPUT_FOLDER below), which matters once this is
installed to Program Files rather than run from a personal folder.
...
# When frozen by PyInstaller, __file__ points into a temp/install extraction
# location - use the real .exe's location instead, so things like the logo
# and template that ship next to the app are still found correctly.
if getattr(sys, "frozen", False):
    APP_DIR = Path(sys.executable).resolve().parent
else:
    APP_DIR = Path(__file__).resolve().parent

# Deliberately NOT next to APP_DIR: once this is MSI-installed to
# C:\Program Files\... (the Intune deployment path), standard users cannot
# write there, so every export would fail with a permissions error. Output
# files instead go to a per-user location that's always writable, regardless
# of where the app itself is installed.
_local_appdata = os.environ.get("LOCALAPPDATA")
USER_DATA_DIR = (
    Path(_local_appdata) / "VAT Compliance Automation"
    if _local_appdata
    else Path.home() / "VAT Compliance Automation"
)
OUTPUT_FOLDER = USER_DATA_DIR / "outputs"
